...
class AuditableBehavior extends ModelBehavior {
  ...
  public function setup( Model $Model, $settings = array() ) {
	  if( $Model->alias == 'Audit' || $Model->alias == 'Sku' || $Model->alias == 'ProductOptions' ) return true;
  ...
  /*  if( count( $updates ) ) {
      foreach( $updates as $delta ) {
        $delta['Audit']['id'] = $Model->Audit->id;

        //$Model->Audit->AuditDelta->create();
        $Model->Audit->save( $delta );

        if( !$created && $Model->hasMethod( 'afterAuditProperty' ) ) {
          $Model->afterAuditProperty(
            $Model,
            $delta['Audit']['property_name'],
            $this->_original[$Model->alias][$delta['Audit']['property_name']],
            $delta['Audit']['new_value']
          );
        }
      }
    }
